% RUN_EMPIRICAL_V1  Chapter 7 v1 driver (LEGACY): block-adaptive BLP on euro-area
% data with the surprise INSIDE the VAR.
%
% *** This file was empirical/RUN_EMPIRICAL.m until 2026-09-15.  It lives in
% *** empirical/legacy_v1/ with its data builders (assemble_dataset,
% *** build_shock_series, ea_relevance) and writes to results/empirical/legacy_v1/.
% *** The current design is empirical/RUN_EMPIRICAL_IV.m.
%
...
% *** RUN_EMPIRICAL_IV.m; see docs/DESIGN.md.  Outputs of this file
% *** keep their p12* names; the v2 driver writes iv_* files.
%
% Design document: docs/DESIGN_V1_LEGACY.md.  Integration notes, data status and
% verified repo signatures: README.md.
%
...
%   C  dose-response in p in {2, 6, 12} (Leg 3; see DESIGN_V1_LEGACY.md Sec. 5.2)
...
% rerun.  B, D and E read results/empirical/legacy_v1/empirical_<tag>.mat, which step A writes;
...
if isempty(ea_this), ea_this = fullfile(pwd, 'RUN_EMPIRICAL_V1'); end
addpath(genpath(fileparts(fileparts(fileparts(ea_this)))));   % <repo>/empirical/legacy_v1 -> <repo>
P = ea_paths();
P.results = P.results_legacy;                 % v1 outputs: results/empirical/legacy_v1
if exist(P.figures_legacy, 'dir') ~= 7, mkdir(P.figures_legacy); end

% --- configuration -----------------------------------------------------------
assert(exist(P.dataset_legacy, 'file') == 2, ...
       ['RUN_EMPIRICAL: %s not found.\n' ...
        'Run the prerequisites first:\n' ...
        '    build_shock_series(); fetch_outcome_data(); assemble_dataset();\n' ...
        'then SMOKE_TEST_EMPIRICAL_V1 before launching this.'], P.dataset_legacy);
ds  = load(P.dataset_legacy);
...
% Early-horizon window of the tau_bar statistic (DESIGN_V1_LEGACY.md Sec. 5.1).
...
fprintf('RUN_EMPIRICAL: %s\n  T = %d, K = %d, p = %d, H = %d, early-h window [%d %d], variant = %s\n  h1_mode = %s, psi_floor = %d, instrument block held at tau = 1: %d, pooled = %d, tag = %s\n', ...
        P.dataset_legacy, size(ds.Y, 1), K, cfg.p, cfg.H, h_early(1), h_early(2), ...
...
try
    ...
    print(fullfile(P.figures_legacy, sprintf('fig_irf_%s.png', tag)), '-dpng', '-r110');
    ...
    print(fullfile(P.figures_legacy, sprintf('fig_lambda_%s.png', tag)), '-dpng', '-r110');
catch err
...
try
    for e = 1:size(ests, 1)
        ...
        print(fullfile(P.figures_legacy, sprintf('fig_tau_heatmap_%s_%s.png', ests{e, 1}, tag)), '-dpng', '-r110');
    end
catch err
    fprintf('  (plotting skipped: %s)\n', err.message);
end
...
[dpth, dnm, dext] = fileparts(P.dataset_legacy);
...
% --- E1. mechanical coherence (DESIGN_V1_LEGACY.md Sec. 5.3) ----------------------
...
% --- E2. the reading protocol (DESIGN_V1_LEGACY.md Sec. 5.1) ----------------------
...
try
    for e = 1:size(ests4, 1)
        ...
        print(fullfile(P.figures_legacy, sprintf('fig_tau_heatmap_levels_%s_%s.png', ests4{e, 1}, tag)), '-dpng', '-r110');
    end
catch err
    fprintf('  (plotting skipped: %s)\n', err.message);
end
...
% The pre-registered reading protocol (DESIGN_V1_LEGACY.md Sec. 5.1) applied to one
