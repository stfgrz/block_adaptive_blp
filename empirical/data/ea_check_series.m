...
if any(diff(ym) == 0)
    chk.ok = false;  chk.fatal = true;
    chk.msgs{end + 1} = sprintf( ...
        ['%s: duplicated TIME_PERIOD values -- the file holds MORE THAN ONE ' ...
         'SERIES (an export with the geo/activity filter left open). No need ' ...
         'to re-download: ea_extract_series(PATH) lists the dimensions, then ' ...
         'ea_extract_series(PATH, PATH, struct(...)) reduces it in place.'], name);
end
