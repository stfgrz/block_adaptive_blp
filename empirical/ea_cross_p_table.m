...
% and escape counts (DESIGN.md Sec. 7).
%
% INPUTS
% ------
...
%     label, estimator, p, null_R, null_method, n_free_cells, n_escape_q95,
%     n_escape_holm (Holm on the Monte Carlo p-values), n_escape_fwer and
%     p_fwer_min (the max-statistic family-wise test), n_eq_flag,
%     max_ratio_q95, median_ratio_q95, top1, top2, top3 ('eq<-block(ratio)'
%     strings, by ratio_q95 over the free cells), key_status (the
%     design-key check of ea_apply_protocol)
%
% NOTES
% -----
...
tab = struct('label', {}, 'estimator', {}, 'p', {}, 'null_R', {}, 'null_method', {}, ...
             'n_free_cells', {}, 'n_escape_q95', {}, 'n_escape_holm', {}, ...
             'n_escape_fwer', {}, 'p_fwer_min', {}, 'n_eq_flag', {}, ...
...
for d = 1:numel(spec)
    ...
    for e = 1:numel(prot.names)
        ...
        row = struct('label', sp.label, 'estimator', nm, 'p', p, ...
                     'null_R', E.null_R, 'null_method', E.null_method, ...
                     'n_free_cells', E.n_free_cells, 'n_escape_q95', E.n_escape_q95, ...
                     'n_escape_holm', E.n_escape_holm, 'n_escape_fwer', E.n_escape_fwer, ...
                     'p_fwer_min', E.p_fwer_min, 'n_eq_flag', E.n_eq_flag, ...
...
% --- print ------------------------------------------------------------------
fprintf('\n%-22s %-7s %3s %5s %-8s %5s %6s %6s %6s %7s %6s %8s %8s  %-22s %-22s %-22s %s\n', ...
        'design', 'est', 'p', 'R', 'innov', 'free', 'q95', 'holm', 'fwer', 'p_fwer', 'eqflag', 'maxrat', 'medrat', ...
        'top1', 'top2', 'top3', 'key');
for k = 1:numel(tab)
    t = tab(k);
    fprintf('%-22s %-7s %3g %5s %-8s %5d %6d %6s %6s %7.3f %6d %8.3f %8.3f  %-22s %-22s %-22s %s\n', ...
            t.label, t.estimator, t.p, num2str(t.null_R), t.null_method, t.n_free_cells, t.n_escape_q95, ...
            num2str(t.n_escape_holm), num2str(t.n_escape_fwer), t.p_fwer_min, t.n_eq_flag, ...
            t.max_ratio_q95, t.median_ratio_q95, t.top1, t.top2, t.top3, t.key_status);
end
...
if ~isempty(out_csv)
    ...
    fprintf(fid, ['design,scale_estimator,p,null_R,null_method,n_free_cells,n_escape_q95,n_escape_holm,' ...
                  'n_escape_fwer,p_fwer_min,n_eq_flag,max_ratio_q95,median_ratio_q95,top1,top2,top3,key_status\n']);
    for k = 1:numel(tab)
        t = tab(k);
        fprintf(fid, '"%s",%s,%g,%s,%s,%d,%d,%s,%s,%.5f,%d,%.5f,%.5f,%s,%s,%s,%s\n', ...
                strrep(t.label, '"', ''''), t.estimator, t.p, num2str(t.null_R), t.null_method, ...
                t.n_free_cells, t.n_escape_q95, num2str(t.n_escape_holm), num2str(t.n_escape_fwer), ...
                t.p_fwer_min, t.n_eq_flag, t.max_ratio_q95, t.median_ratio_q95, ...
