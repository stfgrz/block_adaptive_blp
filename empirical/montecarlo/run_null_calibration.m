...
% WHAT THIS IS
% ------------
% The inferential benchmark ("Leg 2" of DESIGN_V1_LEGACY.md): with real data there
...
%                       conservative side).
%   seed         20260901
%   checkpoint_every 10 save null_ckpt_<stem>.mat next to out_mat every so many reps
%   resume       true   continue from a checkpoint if present
%   dataset      ea_paths().dataset = <repo>/empirical/data/ea_dataset_v2_ois4.mat
%                       (the headline system of the current design; pass
%                       ea_paths().dataset_legacy for the v1 system)
%   tag          ''     extra suffix for the output names (e.g. '_levels')
%   out_mat      <repo>/results/empirical/null_calibration_<stem>.mat
%   out_csv      <repo>/results/empirical/null_thresholds_<stem>.csv
%                       (legacy_v1/ for the v1 dataset)
%                       with <stem> = p<p>[_nofloor][_allblocks]<tag>, the
%                       same tag convention as RUN_EMPIRICAL
%   sim_method   'resample'  residual scheme of simulate_fitted_bvar_dgp:
%                       'resample' (iid rows: homoskedastic null), 'wild'
%                       (Rademacher signs on the SAME month's residual:
%                       keeps the data's volatility profile), 'block'
%                       (moving blocks of 12 months), 'gaussian'.  Any
%                       scheme other than 'resample' is appended to the
%                       stem (e.g. iv_ois4_p12_wild) unless the stem
%                       already ends with it.  The design key does NOT
%                       include the scheme (same statistic, same design):
%                       two nulls for one design differ only in what they
%                       assume about the innovations, which is the point.
...
%       The key is also written to null_calibration_<stem>.key.txt next to the .mat.
...
assert(exist(cfg.null.dataset, 'file') == 2, ...
       ['run_null_calibration: %s not found.\n' ...
        'Build it first: assemble_dataset_v2(struct(''system'', ''ois4'')) for the current design, ' ...
        'or the legacy_v1 builders for ea_dataset.mat.'], cfg.null.dataset);
...
% A non-default residual scheme is always visible in the file name, so a
% wild-bootstrap null can never be mistaken for the iid-resampling one.
if ~strcmp(cfg.null.sim_method, 'resample') && isempty(regexp(stem, ['_' cfg.null.sim_method '$'], 'once'))
    stem = [stem '_' cfg.null.sim_method];
end
% current (instrument-free) designs write to results/empirical, the legacy
% five-variable design to results/empirical/legacy_v1
if instrument_free, out_dir = P.results; else, out_dir = P.results_legacy; end
if isempty(cfg.null.out_mat)
    cfg.null.out_mat = fullfile(out_dir, sprintf('null_calibration_%s.mat', stem));
end
if isempty(cfg.null.out_csv)
    cfg.null.out_csv = fullfile(out_dir, sprintf('null_thresholds_%s.csv', stem));
end
...
if exist(fileparts(cfg.null.out_mat), 'dir') ~= 7, mkdir(fileparts(cfg.null.out_mat)); end
...
ck = fullfile(fileparts(cfg.null.out_mat), sprintf('null_ckpt_%s.mat', stem));   % next to the output
