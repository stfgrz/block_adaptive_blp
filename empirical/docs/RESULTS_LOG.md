# Chapter 7 — chronological results log

> This file is the dated record of every run of the empirical chapter, kept
> verbatim: the v1 run of 2026-09-12 (sections 1–10), the v2 preview on the
> legacy indicator of 2026-09-13 (V2.x) and the first run on the 1-month
> OIS of 2026-09-14 (M.x). The **current** reading of the headline system,
> with the revised protocol and the heteroskedasticity-robust null of
> 2026-09-15, is `RESULTS.md`; where the two disagree, `RESULTS.md` wins
> (in particular the "Holm-significant" statements below rest on an
> `R = 200` null and a p-value convention that `RESULTS.md` corrects).
> File names below are as they were at the time; the v1 files now live in
> `results/empirical/legacy_v1/`, the `iv_*` files in `results/empirical/`.
...
is in `../README.md` (Status 1–3); the pre-registered design and
expectations are in `DESIGN_V1_LEGACY.md`.
...
## 9. Against the pre-registered expectations (`DESIGN_V1_LEGACY.md` Sec. 7)
...
when a white-noise instrument sits inside the system (`../README.md`,
...
design (`DESIGN.md`) pairs a 1-month OIS surprise with a 1-month OIS
...
5. **Not chosen, but stronger:** the 3-month surprise (F = 19.2, the
   ...
   1M instrument; the 3M is the maturity robustness of `DESIGN.md` Sec. 2.
