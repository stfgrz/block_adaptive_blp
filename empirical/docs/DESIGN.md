# Chapter 7 design (current): external-instrument identification, 1-month OIS, and null-calibrated validation

> This is the design in force (written 2026-09-13 as `CH7_REDESIGN.md`, run
> on the real 1-month OIS data on 2026-09-14, protocol revised 2026-09-15).
> The v1 design it replaced (surprise inside the VAR) is
> `DESIGN_V1_LEGACY.md`; the results are in `RESULTS.md` (current) and
> `RESULTS_LOG.md` (chronological); the data in `DATA.md`; the code is
> `RUN_EMPIRICAL_IV.m`. Section 7 below predates the 2026-09-15 protocol
> revision (Monte Carlo and family-wise p-values, wild-bootstrap null):
> see the addendum at the end.

Literature support for every material choice, with what was verified against
the papers' text and what is from memory, is in `LITERATURE.md`. This
...
global tightness down (`RESULTS_LOG.md` Section 2 and `DESIGN_V1_LEGACY.md`). The v1 result stays as
the legacy baseline.
...
`legacy_v1/RUN_EMPIRICAL_V1.m` and every `p12*` output (now in
`results/empirical/legacy_v1/`) are unchanged and labelled "legacy baseline
(v1, internal instrument)". The new driver is
...
## Addendum (2026-09-15): protocol revision

Three changes to Section 7, made after the first reading of the `ois4` map:

1. **Monte Carlo p-values.** The per-cell p-value is `p_mc = (r + 1)/(R + 1)`
   with `r` the number of null draws at or above `tau_bar`, never the naive
   `r/R` that can be exactly zero. Holm's step-down runs on `p_mc`; with
   `m = 16` free cells it can only reject when `R >= 319`, so the `R = 200`
   nulls of 2026-09-14 could not support the Holm claims made then, and the
   headline iid null was rerun with `R = 1000`.
2. **Family-wise max statistic.** In every null draw the largest
   `tau_bar / q95` over the free cells is recorded; `p_fwer(i, g)` is the
   share of draws whose maximum reaches the cell's ratio. This is the
   single-step max-T adjustment of Westfall–Young on the joint null the
   calibration already provides, and it is the family-wise test the chapter
   reads (`ea_apply_protocol.m`).
3. **A heteroskedasticity-robust null.** The iid residual resampling makes
   the simulated innovations homoskedastic, while the euro-area short-rate
   innovation has a four times smaller standard deviation after 2012 than in
   2001–2011. `run_null_calibration(..., 'sim_method', 'wild')` keeps each
   month's own residual and randomises its sign (Rademacher wild
   bootstrap), so the null then describes a correctly specified VAR with
   the data's volatility profile. Both nulls are read; an escape that only
   the homoskedastic null rejects is reported as such.

Also new: `ea_relevance_iv` reports the *anatomy* of the first stage (the
share of the first-stage covariance by year, by sub-period and by the five
largest months) and the predictability of the instrument from the VAR's own
lags, and the pooled estimator is re-run at `H = h_early(2)` for the
protocol because the null is simulated at that `H`.
