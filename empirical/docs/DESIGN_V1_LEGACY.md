# Chapter 7 — v1 design (LEGACY): the surprise inside the VAR
## "Where does the VAR prior fail in euro-area monetary transmission?"

> **Superseded.** This is the design of 2026-09-01 as run on 2026-09-12
> (internal instrument ordered first, 12-month Euribor indicator). It was
> replaced by the external-instrument design in `DESIGN.md` because the
> first stage was weak (robust t 0.77) and the surprise block inside the
> VAR forced two departures from the published FMAR prior. Code:
> `empirical/legacy_v1/`; outputs: `results/empirical/legacy_v1/`; results:
> the first part of `RESULTS_LOG.md`. Kept for the record and as the
> robustness check for invertibility.

Status: first full draft (2026-09-01). Companion code lives in
`empirical/`: `data/`, `dgp/`, `estimators/`, `montecarlo/`,
`legacy_v1/RUN_EMPIRICAL_V1.m`, `legacy_v1/SMOKE_TEST_EMPIRICAL_V1.m`. Run order,
input validation and verified estimator signatures: `../README.md`.
...
> documented in full in the v1 README of 2026-09-12 (now `RESULTS_LOG.md`, sections 1–2, and `../legacy_v1/`):
...
> everyone else. Results: `RESULTS_LOG.md`.
>
...
> `DESIGN.md`, with sources in `LITERATURE.md`. One correction to
...
test. `legacy_v1/RUN_EMPIRICAL_V1.m` step D emits the side-by-side early-h tau table.
...
1. `build_shock_series` + `fetch_outcome_data` + `assemble_dataset` —
   minutes; verify first-stage prints.  Then `SMOKE_TEST_EMPIRICAL_V1`
