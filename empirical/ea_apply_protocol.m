...
% PURPOSE
% -------
% Applies the reading protocol of CH7_DESIGN Sec. 5.1 / CH7_REDESIGN Sec. 7
...
%     percentile of tau_bar among the R null draws, and the one-sided
%     p-value p_cell = 1 - null_pct;
%   * per estimator, the escape COUNTS: cells above q95, cells rejected by
%     Holm's step-down at 5% over the free cells, and equations whose
%     max_g tau_bar exceeds the null max-statistic q95.
...
%                   .null_pct, .p_cell, .escape_q95, .holm_reject (K x G),
%                   .eq_max, .eq_argmax, .eq_flag (K x 1), and the counts
%                   .n_escape_q95, .n_escape_holm, .n_eq_flag, .n_free_cells
%
% NOTES
% -----
% * csv columns: those of RUN_EMPIRICAL's tau_protocol csv, followed by
%   ratio_q95,null_pct,p_cell,holm_reject.  null_pct is the fraction of
%   null draws STRICTLY below tau_bar; it needs nl.<name>.tau_bar_draws
%   (R x K x G) and is NaN when the null does not carry the draws.
% * Holm: the free cells (all equations x free blocks) are ranked by
%   p_cell; cell j is rejected if every cell with a smaller or equal
%   p-value satisfies p <= alpha / (m - rank + 1).  With R null draws the
%   smallest attainable p is 0, so with few draws the correction is coarse.
...
if ~isempty(out_csv)
    ...
    fprintf(fid, ['scale_estimator,equation,block,block_held_at_1,tau_bar,null_mean,null_q90,null_q95,' ...
                  ...
                  'eq_maxstat,eq_maxstat_q95,eq_flag,ratio_q95,null_pct,p_cell,holm_reject\n']);
end
...
for e = 1:size(sets, 1)
    ...
    say('  [%s] reading protocol (null R = %d, p = %d, window h = %d..%d):\n', ...
        nm, n_rep, p_null, h_early(1), h_early(2));
    ...
    holm = holm_stepdown(pcell, free, opts.alpha);
    ...
    for i = 1:K
        ...
        if fid > 0
            for g = 1:K
                ...
                fprintf(fid, '%s,%s,%s,%d,%.5f,%.5f,%.5f,%.5f,%.5f,%d,%.4f,%.4f,%d,%.4f,%.5f,%.5f,%d,%.5f,%.4f,%.4f,%d\n', ...
                        ...
                        ratio(i, g), npct(i, g), pcell(i, g), holm(i, g));
            end
        end
    end
    n_holm = sum(holm(:));
    pf = pcell(:, free);
    if all(isnan(pf(:))), n_holm = NaN; end
    say('    counts: %d free cells | %d above q95 | %d Holm-rejected (%.0f%%) | %d of %d equations flagged\n', ...
        n_free_cells, sum(esc(:)), n_holm, 100 * opts.alpha, sum(eq_flag), K);

    E = struct('tau_bar', tb, 'pgt1_bar', pb, 'null_q95', S.q95, 'ratio_q95', ratio, ...
               'null_pct', npct, 'p_cell', pcell, 'escape_q95', esc, 'holm_reject', holm, ...
               'eq_max', eq_max, 'eq_argmax', eq_am, 'eq_flag', eq_flag, ...
               'n_free_cells', n_free_cells, 'n_escape_q95', sum(esc(:)), ...
               'n_escape_holm', n_holm, 'n_eq_flag', sum(eq_flag));
    prot.(nm) = E;
    prot.names{end + 1} = nm;
end
if fid > 0
